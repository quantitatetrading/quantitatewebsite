 ...
 */
function registerUserInDB(username, password, name, email, registrationCallback){

  //create query using the data in the req.body to register the user in the db
  const registerUserQuery = `INSERT INTO users (username, user_password, email, fullname) VALUES ('${username}', SHA('${password}'),'${email}','${name}')`
...
function getUserFromCrentials(username, password, callback) {
  ...
  OR (email = '${username}' AND user_password = SHA('${password}'))
  `
  ...
  mySqlConnection.query(getUserQuery, (dataResponseObject) => {
      ...
      callback(dataResponseObject.results !== null && dataResponseObject.results.length  === 1 ?  dataResponseObject.results[0] : null)
  })
}
