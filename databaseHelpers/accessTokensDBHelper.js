...
function saveRefreshToken(refreshToken, userID, callback) {
  ...
  const getUserQuery =  `INSERT INTO access_tokens (refresh_tokens, user_id) VALUES ("${refreshToken}", ${userID}) ON DUPLICATE KEY UPDATE refresh_tokens = "${refreshToken}";`
...
function getUserData(token, callback){
  ...
  getUserIDFromBearerToken(token, (id) => {
    console.log(id)
    const query = `SELECT username , email , fullname FROM users WHERE id = '${id}'`
    mySqlConnection.query(query, (dataResponseObject) => {
      console.log(dataResponseObject.results[0].email)
      callback(dataResponseObject.results,dataResponseObject.error)
